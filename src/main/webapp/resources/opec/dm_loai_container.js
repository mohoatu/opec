...
function newItem() {
	...
	$("#dialog").jqxWindow('setTitle', "Thêm loại container");
...
function editItem() {
	...
	$("#dialog").jqxWindow('setTitle', "Sửa thông tin loại container");
    $("#dialog").jqxWindow('open');
    $("#dialog").attr('data-row', row_index);
    $("#jqxDataTable").jqxDataTable({ disabled: true });
    
    $("#container_ma").jqxInput('val', rowData.container_ma);
	$("#container_ten").jqxInput('val', rowData.container_ten);
...
function deleteItem() {
	...
	var url = './dm_loai_container/delete';
	$.ajax({
	    url: url,
	    type: 'POST',
	    dataType: 'json',
...
function initControl() {
	...
	$("#jqxDataTable").jqxDataTable(
	{
		...
		pageSize: 20,
		columns: [
		          { text: 'STT', datafield: 'container_id', width: 60 },
		          { text: 'Mã cont', datafield: 'container_ma', width: 100 },
		          { text: 'Tên cont', datafield: 'container_ten' }
		]
	});
	...
	$("#cmdSave").mousedown(function () {
	    ...
	    if(editRow == -1) {
	    	rowData = {};
	    	rowData.container_id = 0;
	    }
	    
	    rowData.container_ma = $("#container_ma").jqxInput('val');
	    rowData.container_ten = $("#container_ten").jqxInput('val');
	    
	    var url = './dm_loai_container/update';
	    if(editRow == -1) {
	    	url = './dm_loai_container/insert';
	    }
...
function initDanhmuc() {
	...
	$("#container_ma").jqxInput({ height: 25, width: 100 });
	$("#container_ten").jqxInput({ height: 25, width: 300 });
