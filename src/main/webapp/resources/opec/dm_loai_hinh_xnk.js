...
function newItem() {
	...
	$("#dialog").jqxWindow('setTitle', "Thêm loại hình xuất nhập khẩu");
...
function editItem() {
	...
	$("#dialog").jqxWindow('setTitle', "Sửa thông tin loại hình xuất nhập khẩu");
    $("#dialog").jqxWindow('open');
    $("#dialog").attr('data-row', row_index);
    $("#jqxDataTable").jqxDataTable({ disabled: true });

    $("#loaihinhxnk_ma").jqxInput('val', rowData.loaihinhxnk_ma);
	$("#loaihinhxnk_ten").jqxInput('val', rowData.loaihinhxnk_ten);
}
...
function deleteItem() {
	...
	var url = './dm_loai_hinh_xnk/delete';
	$.ajax({
	    url: url,
	    type: 'POST',
	    dataType: 'json',
...
function initControl() {
	...
	$("#jqxDataTable").jqxDataTable(
	{
		...
		pageSize: 20,
		columns: [
		          { text: 'STT', datafield: 'loaihinhxnk_id', width: 60 },
		          { text: 'Mã loại hình XNK', datafield: 'loaihinhxnk_ma', width: 250 },
		          { text: 'Tên loại hình XNK', datafield: 'loaihinhxnk_ten' }
		]
	});
	...
	$("#cmdSave").mousedown(function () {
	    ...
	    if(editRow == -1) {
	    	rowData = {};
	    	rowData.loaihinhxnk_id = 0;
	    }
	    
	    rowData.loaihinhxnk_ma = $("#loaihinhxnk_ma").jqxInput('val');
	    rowData.loaihinhxnk_ten = $("#loaihinhxnk_ten").jqxInput('val');
	    
	    var url = './dm_loai_hinh_xnk/update';
	    if(editRow == -1) {
	    	url = './dm_loai_hinh_xnk/insert';
	    }
	    ...
	    $.ajax({
		    ...
		    success: function(response) {
		    	...
		    	if(editRow != -1) {
			    	...
			    	rowData.loaihinhxnk_id = response;
...
function initDanhmuc() {
	...
	$("#loaihinhxnk_ma").jqxInput({ height: 25, width: 100 });
	$("#loaihinhxnk_ten").jqxInput({ height: 25, width: 300 });
...
//-------------------------------------------------------------------------------
function dmLoaiHinhXnk() {
	
	var key = "";
	var url = "./dm_loai_hinh_xnk/get";
	...
	var source =
	{
			...
			dataFields: [
			             ...
			             { name: 'loaihinhxnk_ma', type: 'string' },
			             { name: 'loaihinhxnk_ten', type: 'string' }
			             ],
			id: 'loaihinhxnk_id',
			url: url
	 };
