...
function newItem() {
    ...
    $("#jqxDataTableCodeHD").jqxDataTable({ disabled: true });
...
function editItem() {
	...
	var rows_selected = $("#jqxDataTableCodeHD").jqxDataTable('getSelection');
	...
	if (confirm('Bạn có chắc chắn muốn xóa?')) {
		...
		$.ajax({
		    ...
		    success: function(response) {
		    	...
		    	//alert('success');
		    	$("#jqxDataTableCodeHD").jqxDataTable('deleteRow', row_index);
		    },
...
//-------------------------------------------------------------------------------
function exportXls() {
	$("#jqxDataTableCodeHD").jqxDataTable({exportSettings:  { fileName: "Hợp đồng nhập khẩu"}});
	$("#jqxDataTableCodeHD").jqxDataTable('exportData', 'xls');
...
//-------------------------------------------------------------------------------
function initControlHopDong() {
	
	$('#jqxDataTableCodeHD').on('rowSelect', 
			 function (event)
			 {
			     // event args.
			     var args = event.args;
...
//-------------------------------------------------------------------------------
function xnk_nk_hop_dong_quichinh() {
	...
	$("#jqxDataTableCodeHD").jqxDataTable(
	{
		theme: 'energyblue',
		width: "100%",
		height: 600,
...
function controlResize() {
	...
	$('#jqxDataTableCodeHD').jqxDataTable({height: height.toString() + "px"});
}
...
function add_update_nk_hop_dong(){
   ...
   // close jqxWindow.
    ...
    $.ajax({
	    ...
	    success: function(response) {
	    	...
	    	if(editRow != -1) {
		    	$("#jqxDataTableCodeHD").jqxDataTable('updateRow', editRow, rowData);
		    } else {
		    	rowData.hopdong_id = response;
		    	$("#jqxDataTableCodeHD").jqxDataTable('addRow', response, rowData); // Tham so thu 2 la null thi ID++
		    }
 ...
 function doWhenCloseWindow(){
 		    // enable jqxDataTable.
	    $("#jqxDataTableCodeHD").jqxDataTable({ disabled: false });
	    //clear input data
