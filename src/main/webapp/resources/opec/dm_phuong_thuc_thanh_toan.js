...
function newItem() {
	...
	$("#dialog").jqxWindow('setTitle', "Thêm phương thức thanh toán");
...
function editItem() {
	...
	$("#dialog").jqxWindow('setTitle', "Sửa thông tin phương thức thanh toán");
    $("#dialog").jqxWindow('open');
    $("#dialog").attr('data-row', row_index);
    $("#jqxDataTable").jqxDataTable({ disabled: true });
    
    $("#phuongthuctt_ma").jqxInput('val', rowData.phuongthuctt_ma);
	$("#phuongthuctt_ten").jqxInput('val', rowData.phuongthuctt_ten);
...
function deleteItem() {
	...
	var url = './dm_phuong_thuc_thanh_toan/delete';
	$.ajax({
	    url: url,
	    type: 'POST',
	    dataType: 'json',
...
function initControl() {
	...
	$("#jqxDataTable").jqxDataTable(
	{
		...
		pageSize: 20,
		columns: [
		          { text: 'STT', datafield: 'phuongthuctt_id', width: 60 },
		          { text: 'Mã PTTT', datafield: 'phuongthuctt_ma', width: 100 },
		          { text: 'Tên phương thức thanh toán', datafield: 'phuongthuctt_ten' }
		]
	});
	...
	$("#cmdSave").mousedown(function () {
	    ...
	    if(editRow == -1) {
	    	rowData = {};
	    	rowData.phuongthuctt_id = 0;
	    }
	    
	    rowData.phuongthuctt_ma = $("#phuongthuctt_ma").jqxInput('val');
	    rowData.phuongthuctt_ten = $("#phuongthuctt_ten").jqxInput('val');
	    
	    var url = './dm_phuong_thuc_thanh_toan/update';
	    if(editRow == -1) {
	    	url = './dm_phuong_thuc_thanh_toan/insert';
	    }
	    ...
	    $.ajax({
		    ...
		    success: function(response) {
		    	...
		    	if(editRow != -1) {
			    	...
			    	rowData.phuongthuctt_id = response;
...
function initDanhmuc() {
	...
	$("#phuongthuctt_ma").jqxInput({ height: 25, width: 100 });
	$("#phuongthuctt_ten").jqxInput({ height: 25, width: 300 });
...
function dmPhuongThucThanhToan() {
	...
	var source =
	{
			...
			dataFields: [
			             ...
			             { name: 'phuongthuctt_ten', type: 'string' }
			             ],
			id: 'phuongthuctt_id',
			url: url
	 };
