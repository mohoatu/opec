...
function newItem() {
	...
	$("#dialog").jqxWindow('setTitle', "Thêm loại thuế suất");
...
function editItem() {
	...
	$("#dialog").jqxWindow('setTitle', "Sửa thông tin loại thuế suất");
    $("#dialog").jqxWindow('open');
    $("#dialog").attr('data-row', row_index);
    $("#jqxDataTable").jqxDataTable({ disabled: true });

    $("#ty_le_thue_suat").jqxNumberInput('val', rowData.ty_le_thue_suat);
	$("#thuesuat_ten").jqxInput('val', rowData.thuesuat_ten);   
}
...
function initControl() {
	...
	$("#jqxDataTable").jqxDataTable(
	{
		...
		pageSize: 20,
		columns: [
		          { text: 'STT', datafield: 'thuesuat_id', width: 100 },
		          { text: 'Tỷ lệ thuế suất', datafield: 'ty_le_thue_suat', width: 200 },
		          { text: 'Tên thuế suất', datafield: 'thuesuat_ten' }
		]
	});
	...
	$("#cmdSave").mousedown(function () {
	    ...
	    if(editRow == -1) {
	    	rowData = {};
	    	rowData.thuesuat_id = 0;
	    }
	    
	    rowData.ty_le_thue_suat = $("#ty_le_thue_suat").jqxNumberInput('val');
	    rowData.thuesuat_ten = $("#thuesuat_ten").jqxInput('val');
	    
	    var url = './dm_thue_suat/update';
	    if(editRow == -1) {
	    	url = './dm_thue_suat/insert';
	    }
...
function initDanhmuc() {
    ...
    $("#ty_le_thue_suat").jqxNumberInput({ width: 100, height: 25, digits: 4, decimalDigits: 2, spinButtons: false, symbolPosition: 'right', symbol: '%', groupSeparator: " " });
	$("#thuesuat_ten").jqxInput({ height: 25, width: 300 });
...
function dmThueSuat() {
	...
	var source =
	{
			dataType: "json",
			dataFields: [
			             { name: 'thuesuat_id', type: 'int' },
			             { name: 'ty_le_thue_suat', type: 'double' },
			             { name: 'thuesuat_ten', type: 'string' }
			             ],
			id: 'thuesuat_id',
			url: url
	 };
