...
function newItem() {
	...
	$("#dialog").jqxWindow('setTitle', "Thêm quốc gia");
...
function editItem() {
	...
	$("#dialog").jqxWindow('setTitle', "Sửa thông tin quốc gia");
    $("#dialog").jqxWindow('open');
    $("#dialog").attr('data-row', row_index);
    $("#jqxDataTable").jqxDataTable({ disabled: true });
    
    $("#quocgia_ten").jqxInput('val', rowData.quocgia_ten);
...
function initControl() {
	...
	$("#jqxDataTable").jqxDataTable(
	{
		...
		pageSize: 20,
		columns: [
		          { text: 'STT', datafield: 'quocgia_id', width: 100 },
		          { text: 'Tên quốc gia', datafield: 'quocgia_ten' }
		]
	});
	...
	$("#cmdSave").mousedown(function () {
	    ...
	    rowData.quocgia_ten = $("#quocgia_ten").jqxInput('val');
	    
	    var url = './dm_quoc_gia/update';
	    if(editRow == -1) {
	    	url = './dm_quoc_gia/insert';
	    }
...
function initDanhmuc() {
	...
	$("#quocgia_ten").jqxInput({ height: 25, width: 300 });
